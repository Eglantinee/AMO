...
def check_d(elem):
    if not str(elem).replace(".", "").isdigit():
        return False
    return True
...
def run(lst, self):
    if check(lst):
        if float(lst[1].get()) != 0:
            ...
            self.config(text='y1= ' + str(round(res, ndigits=4)))
        else:
    ...
    def open_file():
        ...
        if fl is not None:
            ...
            for i in range(len(items)):
                if items[i].lower() == 'a' and check_d(values[i]) is True:
                    a = float(values[i])
                elif items[i].lower() == 'b' and check_d(values[i]) is True:
                    if float(values[i]) == 0:
                        mb.showerror(title="Error", message="b should not be equal 0")
                    else:
                        b = float(values[i])
                elif items[i].lower() == 'c' and check_d(values[i]) is True:
                    c = float(values[i])
                elif items[i].lower() == 'd' and check_d(values[i]) is True:
                    d = float(values[i])
                elif items[i].lower() == 's' and check_d(values[i]) is True:
                    s = float(values[i])
